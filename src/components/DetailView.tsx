...
export function DetailView({ pokeData, speciesData, evoChain }: DetailViewProps) {
  return (
    <Stack spacing={3} sx={{ width: "100%", maxWidth: 900 }}>
      <PokemonCard pokeData={pokeData} speciesData={speciesData} />
      <DetailCard>
        <DetailBody pokeData={pokeData} speciesData={speciesData} evoChain={evoChain} />
      </DetailCard>
    </Stack>
  );
}
...
export function DetailBody({ pokeData, speciesData, evoChain }: DetailBodyProps) {
  ...
  return (
    <>
      ...
      <Grid
        container
        spacing={2.5}
        sx={{ textAlign: "left", my: 4, p: 2.5, bgcolor: "#f9f9f9", borderRadius: "20px" }}
      >
            ...
            {evoChain.map((step, i) => (
              <Stack key={step.id} direction="row" spacing={2.5} sx={{ alignItems: "center" }}>
                ...
                >
                  ...
                  />
                  <Typography
                    sx={{ textTransform: "capitalize", fontWeight: "bold", fontSize: 14, mt: 1 }}
                  >
                    {step.name}
                  </Typography>
                </LinkedButtonBase>
